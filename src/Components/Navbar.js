    ...
    >
      ...
      >
        <img
          src={assets.logo}
          alt="Company Logo"
          className="min-w-36 h-7 transition-transform group-hover:scale-105"
        />
      ...
      <ul className="hidden sm:flex gap-5 text-base font-medium text-[#4A5568] text-nowrap mx-5">
        {navLinks.map((link) => (
          ...
          >
            <p>{link.label}</p>
            <motion.hr
              className="w-2/4 h-[2px] bg-blue-600 border-none"
              initial={{ width: 0 }}
      ...
      <button
        ...
        className="hidden sm:block text-white font-semibold bg-gradient-to-r from-blue-700 to-sky-500 hover:from-blue-900 hover:to-sky-600 px-8 py-2 rounded-full h-10 transition-all duration-300 transform hover:scale-105 hover:shadow-lg"
      >
        Contact us
      </button>
      ...
      >
        <div className="flex flex-col bg-white  text-gray-600 h-full">
          ...
          <ul className="flex flex-col  items-center mt-5 text-lg font-medium">
            {navLinks.map((link, index) => (
              ...
              >
                <button
                  onClick={() => scrollToSection(link.id)}
                  className={`px-6 py-3 rounded-full inline-block hover:bg-blue-50 hover:text-blue-600 transition-all ${
          ...
          <motion.button
            ...
            onClick={() => scrollToSection("contact")}
            className="mx-6 mt-8 text-white font-semibold bg-gradient-to-r from-blue-600 to-blue-600 px-8 py-3 rounded-full transition-all duration-300"
          >
            Contact us
          </motion.button>
        </div>
